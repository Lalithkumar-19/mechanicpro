...
const MechanicLogin = () => {
    ...
    useEffect(() => {
        const userInfo = localStorage.getItem('mechanic_token');
        if (userInfo) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsLoading(true);
            const res = await axiosInstance.post('/mechanic/login', {
                phone: formData.phone,
                password: formData.password
            });
            console.log(res,"res");
            if (res.status === 200) {
                const { token, mechanic } = res.data;
                localStorage.setItem('mechanic_token', token);
                localStorage.setItem('mechanic_info', JSON.stringify(mechanic));
